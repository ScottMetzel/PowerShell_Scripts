...
param baseName string = 'la-export'
...
resource resourceGroups 'Microsoft.Resources/resourceGroups@2024-03-01' = [
  for i in range(1, instanceCount): {
    name: 'rg-${baseName}-${padLeft(i, 2, '0')}'
    location: location
  }
]

module automationDeployments './automationAccount.bicep' = [
  for i in range(1, instanceCount): {
    scope: resourceGroups[i - 1]
    params: {
      location: location
      automationAccountName: 'aa-${baseName}-${padLeft(i, 2, '0')}'
...
resource logAnalyticsReaderAssignments 'Microsoft.Authorization/roleAssignments@2022-04-01' = [
  for i in range(1, instanceCount): {
    name: guid(subscription().id, 'aa-${baseName}-${padLeft(i, 2, '0')}', logAnalyticsReaderRoleId)
    properties: {
...
resource storageAccountContributorAssignments 'Microsoft.Authorization/roleAssignments@2022-04-01' = [
  for i in range(1, instanceCount): {
    name: guid(subscription().id, 'aa-${baseName}-${padLeft(i, 2, '0')}', storageAccountContributorRoleId)
    properties: {
      principalId: automationDeployments[i - 1].outputs.principalId
...
resource storageBlobDataContributorAssignments 'Microsoft.Authorization/roleAssignments@2022-04-01' = [
  for i in range(1, instanceCount): {
    name: guid(subscription().id, 'aa-${baseName}-${padLeft(i, 2, '0')}', storageBlobDataContributorRoleId)
    properties: {
      principalId: automationDeployments[i - 1].outputs.principalId
      roleDefinitionId: subscriptionResourceId(
        'Microsoft.Authorization/roleDefinitions',
